...
class EnvironmentConfig {
  ...
  static void validateAll() {
    final configs = ['development', 'production', 'dev', 'prod'];
    var foundAny = false;
    for (final env in configs) {
      final file = File('build_config/$env.json');
      if (file.existsSync()) {
        foundAny = true;
        try {
    ...
    if (!foundAny) {
      stderr.writeln(
        'Error: No build configuration files found in build_config/',
      );
      exit(1);
    }
  }
...
ASSETCATALOG_COMPILER_APPICON_NAME = ${envConfig.iosAppIconName}
''';
      ...
      'client': [
        {
          'client_info': {
            ...
            'android_client_info': {
              'package_name': envConfig.androidAppId,
            },
          },
          'oauth_client': [],
          'api_key': [
...
Future<void> fetch(Config config) async {
  ...
  // Generate AppConfig.xcconfig and app.properties
