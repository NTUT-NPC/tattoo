...
- CampusWifiPlatform — Method-channel client wrapper that exposes Android-only system Wi-Fi APIs (suggestions and settings) to the Dart application.
- Ntut8021xStateStore — Securely stores device-local provisioning state (modes, credential fingerprint, pending compat prompts) using SharedPreferences.
