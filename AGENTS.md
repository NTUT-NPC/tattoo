...
**Credentials:** `tool/credentials.dart` manages encrypted credentials from the `tattoo-credentials` Git repo. Run `dart run tool/credentials.dart fetch` to populate both flavors through a static source-to-destination mapping. Firebase configs live under `firebase/staging/` and `firebase/production/` in the credentials repository and decrypt to flavor-specific ignored paths; signing files are shared. Config comes from env vars or `.env`.

**Build flavors:** Native Flutter flavors are the build-environment source of truth. `staging` is the default and uses `club.ntut.tattoo`, the Tattoo name, and outlined wireframe T icon. `production` uses Android `club.ntut.npc.tat`, iOS `com.npc.tatFlutter`, the TAT name, and filled T icon. PR previews and daily releases MUST build `staging`; production releases MUST build `production`. Use `appFlavor` for Dart-side environment selection and keep Firebase credential files in their flavor-specific ignored paths. `lib/firebase_options_staging.dart` and `lib/firebase_options_production.dart` are FlutterFire-generated; `lib/firebase_options.dart` is the handwritten flavor selector and MUST NOT be overwritten by FlutterFire CLI.
...
**Portal endpoints scraper:** `tool/scrape_portal.dart` scrapes all available subsystem endpoints and their `apOu` codes from the NTUT portal (`nportal.ntut.edu.tw/aptreeMain.do`). It bypasses the web login captcha using the mobile App User-Agent authentication flow and supports outputting the results as plain text or JSON.
...
- CourseWidgetPlatform — Android-only method-channel bridge that atomically persists the latest course-table PNG and routes launcher-widget taps into the authenticated course-table screen. `CourseWidgetSyncController` observes cache transactions, serializes rendering, deduplicates output by fingerprint, and clears native widget state before logout or destructive local-data operations.
...
**Session Lifecycle:** `sessionProvider` (`Notifier<bool>`) drives auth state. `true` = authenticated, `false` = unauthenticated. Router guard watches it for redirect. Repository providers `ref.watch(sessionProvider)` to be recreated with fresh state when the session ends. Startup normally restores the stored `Users` row; if schema recreation removed it, `AuthRepository.restoreSession()` revalidates secure credentials, then old TAT's plaintext `UserDataJsonKey`, and rebuilds the row. Successful migration and logout delete the legacy payload. Transient network failures retain credentials and open login for retry. On auth failure, `withAuth` destroys the session and returns a never-completing `Completer<T>().future` — session-scoped providers are already being disposed by the time callers would stall, so the hanging future is harmless. Callers only need to handle `DioException` for network failures.
