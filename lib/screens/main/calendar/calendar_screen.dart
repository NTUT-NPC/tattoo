      ...
      body: eventsAsync.when(
        ...
        error: (error, _) => CustomScrollView(
          slivers: [
            ...
            SliverFillRemaining(
              hasScrollBody: false,
              child: Center(child: Text('Calendar load failed\n$error')),
            ),
          ],
        ),
        data: (snapshot) {
            ...
            child: CustomScrollView(
              ...
              slivers: [
                ...
                if (snapshot.cachedAt != null)
                  SliverToBoxAdapter(
                    child: Padding(
                      ...
                      child: Text(
                        'Updated ${DateFormat('yyyy/MM/dd HH:mm').format(snapshot.cachedAt!.toLocal())}',
                        style: Theme.of(context).textTheme.bodySmall,
                      ),
                    ),
                  ),
                if (events.isEmpty)
                  const SliverFillRemaining(
                    hasScrollBody: false,
                    child: Center(child: Text('No upcoming calendar events')),
                  )
                else
                  SliverPadding(
